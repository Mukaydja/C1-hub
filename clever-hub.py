...
def to_num(x) -> pd.Series:
    ...
    if isinstance(x, pd.Series):
        ...
        # Même pour un scalaire, retourner une série à 1 élément
...
def calculate_performance_score(player_data):
    ...
    # Efficacité passes — SUM sur la série
    ...
    # Réussite duels — SUM sur la série
    ...
    # Contribution offensive — SUM sur la série
    ...
    # Contribution défensive — SUM sur la série
    ...
    # Rétention du ballon — SUM sur la série
...
def create_radar_chart(data, categories, title="Performance Radar"):
    """Crée un graphique radar pour les performances"""
    fig = go.Figure()
...
def predict_performance_trend_manual(x, y, periods_ahead=5):
    """Régression linéaire manuelle sans sklearn"""
    if len(x) < 2:
        return None
    n = len(x)
    sum_x = np.sum(x)
...
# === FONCTION AJOUTÉE ICI : CALCUL DES 9 KPIS ===
def calculate_kpis(data, total_min, total_matches):
    """Calcule les 9 KPIs personnalisés pour la performance tactique"""
    kpis = {}
    ...
    buts = to_num(buts_col).sum()
    kpis['goals_per_xg'] = buts / xg if xg > 0 else 0  # Efficacité de finition
...
# Normaliser PlayerID en string partout
    ...
    if player_id is not None:
                        ...
                        <div class="glass">
                            <div style="display:flex; gap:12px; align-items:center; margin-bottom: 16px;">
                                ...
                                <div>
                                    ...
                                    <div style="color: var(--muted); font-size: 14px;">{p.get('Poste','')} • {p.get('Club','')}</div>
                                </div>
                            </div>
        ...
        with col2:
            ...
            if not df_match.empty and "PlayerID_norm" in df_match.columns:
                ...
                if not dm.empty:
                    total_minutes = to_num(dm.get("Minute jouee")).sum()
                    total_matches = len(dm)
                    kpis_season = calculate_kpis(dm, total_minutes, total_matches)
                    
                    # Afficher 3 KPIs majeurs
                    cols = st.columns(3)
                    with cols[0]:
                    ...
                    # Normaliser les KPIs pour le radar (0-100)
                    radar_values = [
                        min(kpis_season['pass_accuracy'], 100),
                        min(kpis_season['prog_passes_per_90'] * 10, 100),  # Normalisé (8 -> 80)
                        min(kpis_season['key_passes_per_match'] * 50, 100), # Normalisé (1.5 -> 75)
                        min(kpis_season['shot_accuracy'], 100),
                        min(kpis_season['xg_per_90'] * 150, 100),          # Normalisé (0.5 -> 75)
                        min(kpis_season['goals_per_xg'] * 70, 100),        # Normalisé (1.2 -> 84)
                        min(kpis_season['duel_win_rate'], 100),
                        min(kpis_season['interceptions_per_90'] * 30, 100), # Normalisé (2.5 -> 75)
                        min(kpis_season['recoveries_per_90'] * 10, 100)    # Normalisé (8 -> 80)
                    ]
                    
                    radar_fig = create_radar_chart(radar_values, radar_categories, "Performance Tactique Complète")
                    st.plotly_chart(radar_fig, use_container_width=True)
        
        # === SYNTHÈSE MATCH SPÉCIFIQUE (NOUVEAU DESIGN) ===
        st.markdown("##### 🎯 Synthèse Match Spécifique — Améliorée")
        if not df_match.empty:
            dm = df_match[df_match["PlayerID_norm"] == player_id].copy()
            if not dm.empty and "Journée" in dm.columns:
                # Sélection rapide du dernier match
                last_match = dm.iloc[-1]
                j_day = last_match.get("Journée", "N/A")
                opponent = last_match.get("Adversaire", "N/A")
                
                # Calculer les KPIs pour ce match
                match_df = pd.DataFrame([last_match])
                total_min_scalar = to_num(last_match.get("Minute jouee", 0)).iloc[0]  # Extraire scalaire
                kpis_match = calculate_kpis(match_df, total_min_scalar, 1)
                
                # Wellness du match (si disponible)
                wellness_summary = {}
...
with tabs[1]:
    ...
    if player_id is not None and not df_match.empty:
        dm = df_match[df_match["PlayerID_norm"] == player_id].copy()
        if not dm.empty:
            # Sélection de match spécifique ou vue globale
            ...
            if not match_data.empty:
                total_minutes = to_num(match_data.get("Minute jouee", 0)).sum()
                total_matches = len(match_data) if analysis_mode == "📊 Vue saison complète" else 1
                
                kpis = calculate_kpis(match_data, total_minutes, total_matches)
                ...
                with dist_cols[0]:
                    color = "#10b981" if kpis['pass_accuracy'] > 80 else "#f59e0b" if kpis['pass_accuracy'] > 70 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h3>Précision Passes</h3>
                        <div class="value" style="color: {color};">{kpis['pass_accuracy']:.1f}%</div>
                        <div style="font-size: 12px; color: var(--muted);">>80% = Excellent</div>
                    </div>
                    """, unsafe_allow_html=True)
                
                with dist_cols[1]:
                    color = "#10b981" if kpis['prog_passes_per_90'] > 8 else "#f59e0b" if kpis['prog_passes_per_90'] > 5 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                ...
                with dist_cols[2]:
                    color = "#10b981" if kpis['key_passes_per_match'] > 1.5 else "#f59e0b" if kpis['key_passes_per_match'] > 0.8 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                ...
                with off_cols[0]:
                    color = "#10b981" if kpis['shot_accuracy'] > 40 else "#f59e0b" if kpis['shot_accuracy'] > 30 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h3>Précision Tirs</h3>
                        <div class="value" style="color: {color};">{kpis['shot_accuracy']:.1f}%</div>
                        <div style="font-size: 12px; color: var(--muted);">>40% = Très bon</div>
                    </div>
                    """, unsafe_allow_html=True)
                
                with off_cols[1]:
                    color = "#10b981" if kpis['xg_per_90'] > 0.5 else "#f59e0b" if kpis['xg_per_90'] > 0.3 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h3>xG Généré</h3>
                ...
                with off_cols[2]:
                    color = "#10b981" if kpis['goals_per_xg'] > 1.2 else "#f59e0b" if kpis['goals_per_xg'] > 0.8 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                ...
                with def_cols[0]:
                    color = "#10b981" if kpis['duel_win_rate'] > 55 else "#f59e0b" if kpis['duel_win_rate'] > 50 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h3>Taux Duel Gagné</h3>
                        <div class="value" style="color: {color};">{kpis['duel_win_rate']:.1f}%</div>
                        <div style="font-size: 12px; color: var(--muted);">>55% = Dominant</div>
                    </div>
                    """, unsafe_allow_html=True)
                
                with def_cols[1]:
                    color = "#10b981" if kpis['interceptions_per_90'] > 2.5 else "#f59e0b" if kpis['interceptions_per_90'] > 1.5 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h3>Interceptions</h3>
                ...
                with def_cols[2]:
                    color = "#10b981" if kpis['recoveries_per_90'] > 8 else "#f59e0b" if kpis['recoveries_per_90'] > 5 else "#ef4444"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h3>Récupérations</h3>
                ...
                # === GRAPHIQUE SYNTHÈSE : BARRES HORIZONTALES PAR CATÉGORIE ===
                st.markdown("#### 📊 Synthèse Visuelle des KPIs")
                st.caption("Comparaison par rapport aux benchmarks (valeurs seuils pour 'Excellent')")
                
                # Préparer les données pour le graphique
                categories = ['Distribution', 'Offense', 'Défense']
                kpi_names = [
                ...
                benchmarks = [80, 8, 1.5, 40, 0.5, 1.2, 55, 2.5, 8]  # Seuils pour "Excellent"
                ...
                fig_synthesis.update_layout(
                    title="Performance par KPI vs Benchmark",
                    xaxis_title="Valeur",
                    yaxis_title="KPI",
                    paper_bgcolor='rgba(0,0,0,0)',
                    plot_bgcolor='rgba(0,0,0,0)',
...
# ======================= PROJECTIONS ET TENDANCES (REFONTE ML SANS SKLEARN) =======================
with tabs[2]:
    ...
    if player_id is not None and not df_match.empty and show_predictions:
        dm = df_match[df_match["PlayerID_norm"] == player_id].copy()
        if not dm.empty and len(dm) >= 5:  # Minimum 5 matchs pour entraîner le modèle
            ...
            # Sélectionner les KPIs à prédire
            kpi_options = {
            ...
            for i in range(len(dm_ml)):
                ...
                total_min = to_num(match_slice.get("Minute jouee", 0)).sum()
                total_matches = len(match_slice)
                kpi_dict = calculate_kpis(match_slice, total_min, total_matches)
                historical_kpis.append(kpi_dict[selected_kpi_key])
            
            dm_ml['target_kpi'] = historical_kpis
            
            # Entraîner le modèle de régression linéaire MANUELLE
            X = dm_ml['match_number'].values
            y = dm_ml['target_kpi'].values
            
            # Division train/test (80/20) — manuelle
            n = len(X)
            split_idx = int(0.8 * n)
            ...
            if model:
                # Prédictions sur l'historique + futur
                all_match_numbers = np.arange(1, len(dm_ml) + periods_ahead + 1)
                # Prédire l'historique pour le graphique
                ...
                confidence_interval = 1.96 * mae  # Intervalle à 95%
                ...
                st.warning("⚠️ Impossible de calculer la régression — données insuffisantes.")
        else:
            st.warning("⚠️ Il faut au moins 5 matchs pour entraîner un modèle de prédiction fiable.")

# ======================= WELLNESS AVANCÉ (AVEC COURBES DE TENDANCE) =======================
with tabs[3]:
    ...
    if player_id is not None and not df_well.empty:
        ...
        if not dw.empty and "DATE" in dw.columns:
            dw = dw.sort_values("DATE").tail(60)  # Derniers 60 jours pour une vue claire
            ...
            if wellness_metrics:
                ...
                # === GRAPHIQUE 1 : COURBES INDIVIDUELLES ===
                ...
                if not df_match.empty:
                    ...
                    if "DATE" in dm_player.columns:
                        ...
                        # Fusionner les données sur une fenêtre de 3 jours
                        correlation_data = []
                        for _, match_row in dm_player.iterrows():
                            ...
                            if pd.notna(match_date):
                                ...
                                if not wellness_window.empty:
                                    # Moyenne des métriques wellness
                                    avg_wellness = {metric: wellness_window[metric].mean() for metric in selected_metrics}
                                    # KPI de performance du match
                                    match_df = pd.DataFrame([match_row])
                                    total_min_scalar = to_num(match_row.get("Minute jouee", 0)).iloc[0]
                                    perf_kpis = calculate_kpis(match_df, total_min_scalar, 1)
                                    correlation_data.append({**avg_wellness, **perf_kpis})
                        
                        if len(correlation_data) >= 3:
                            corr_df = pd.DataFrame(correlation_data)
                            
                            # Sélectionner 1 KPI de performance pour la corrélation
...
with tabs[4]:
    ...
    if compare_mode and player_id is not None and compare_player_id is not None:
        ...
        if not dm1.empty and not dm2.empty:
            ...
            st.markdown("##### 📊 KPIs Comparés")
            kpi_cols = st.columns(5)
            # Calculs pour joueur 1
            p1_matches = len(dm1)
            p1_minutes = int(to_num(dm1.get("Minute jouee", 0)).sum())
            ...
            # Calculs pour joueur 2
            p2_matches = len(dm2)
            p2_minutes = int(to_num(dm2.get("Minute jouee", 0)).sum())
            ...
            kpi_cols[0].metric(
                "Matchs Joués",
                f"{p1_matches}",
                f"{p1_matches - p2_matches:+d} vs {player2_name[:10]}"
            )
            kpi_cols[1].metric(
                "Minutes",
                f"{p1_minutes}",
                f"{p1_minutes - p2_minutes:+d}"
            )
            kpi_cols[2].metric(
                "Buts",
                f"{p1_buts}",
            ...
            kpi_cols[4].metric(
                "Passes",
                f"{p1_passes}",
                f"{p1_passes - p2_passes:+d}"
            )
            # === RADAR COMPARISON ===
            st.markdown("##### 🕸️ Comparaison Radar")
            col1, col2 = st.columns(2)
            # Calculs des métriques normalisées pour les radars
            ...
            st.warning("Données insuffisantes pour l'un des joueurs sélectionnés.")
    elif not compare_mode:
        st.info("Activez le 'Mode comparaison' dans la sidebar pour comparer les joueurs.")
    else:
        st.warning("Sélectionnez deux joueurs différents pour activer la comparaison.")
            ...
            for pid in df_match["PlayerID_norm"].unique():
                ...
                if not dm_player.empty:
                    stats = {
                        ...
                        'Minutes_Total': int(to_num(dm_player.get("Minute jouee", 0)).sum()),
...
st.markdown(
    ...
    <div style="text-align: center; padding: 20px; color: var(--muted);">
        ...
        <p style="font-size: 12px;">Version 2.2 • Wellness par courbes • Dashboard amélioré • Sans sklearn</p>
    </div>
    """,
    unsafe_allow_html=True
)
